...
\begin{equation}
\mathcal{F}_t = \Big\{ \lambda_t,\ \text{CF}_t,\ \mathbf{\Psi}_t,\ S_{\text{avail},t},\ \text{Tax}_{\text{Status},t},\ V_t,\ s_t,\ \mathbf{K}_t \Big\}
\end{equation}

其中关键变量修正定义如下：
\begin{itemize}
    \item $\lambda_t = \frac{D_t}{D_t + E_t}$：\textbf{统一杠杆率}。根据题目要求统一采用资产负债口径，与 Section 8 定义保持一致；
    \item $s_t$：\textbf{所有者持股比例}（Owner's Share）。$s_0=100\%$，随股权激励 $a_{\text{equity}}$ 逐年摊薄；
    \item $\mathbf{K}_t$：\textbf{经营配置状态}（Configuration State）。记录当前生效的票价、营销与分红档位，用于保证 Action Masking 的马尔可夫性：
    \begin{equation}
        \mathbf{K}_t = [k_{\text{ticket}},\ k_{\text{marketing}},\ k_{\text{equity}}]
    \end{equation}
...
\hline
\textbf{动作分量} & \textbf{Offseason (休赛期)} & \textbf{Regular (常规赛)} & \textbf{Deadline (交易期)} & \textbf{Playoff (季后赛)} \\ \hline
...
\textbf{掩码机制}：
\begin{equation}
\mathcal{A}_{\text{valid}}(s_t) = \{ \mathbf{a} \in \mathcal{A} \mid \forall j, \text{Mutable}(j, \Theta_t) \text{ or } a_j = \mathbf{K}_t[j] \}
\end{equation}
...
\begin{itemize}
    ...
    初始 $s_0=100\%$。每期股权授予 $a_{\text{equity}}$ 会导致永久性稀释：
    \begin{equation}
...
+ w_{\text{val}} \cdot V_t 
+ w_{\text{win}} \cdot \mathbb{I}(\text{Playoff}) 
...
\begin{itemize}
    ...
    \item $P_{\text{risk}}(\lambda_t)$：统一使用 $\lambda_t = \frac{D_t}{D_t + E_t}$ 计算的风险惩罚项。
\end{itemize}
