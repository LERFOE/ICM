...
# 问题求解与实验结果 (Problem Solving & Results)
...
本节回答题目第 3 问：当联盟在某赛季扩军时，Indiana Fever 的经营与竞技策略应如何从“初始策略”做出调整；不同选址如何改变最优策略；以及扩军下哪些球队所有者更有利/更不利。

为保证结论“可复现且可追溯”，本节每一个公式都对应代码中的可计算对象，并用实验输出文件给出数值证据（见 `project/experiments/output/` 与 `newfigures/`）。

---

### A. 数学模型（Expansion Shock → 竞技/财务传播链 → 价值评估）

#### A1) 输出指标（Indiana 所有者视角）

我们在 Q3 的主要评估指标为终值（所有者权益价值）与经营现金流：
$$
TerminalValue = s_T\cdot(FV_T-D_T).
$$
解释：$FV_T$ 为赛季终点的特许经营权价值（Franchise Value），$D_T$ 为债务余额，$s_T$ 为所有者持股比例（本节实验中关闭股权稀释，因此 $s_T=1$），该终值与 Q1 的杠杆定义 $\lambda_t=D_t/FV_t$ 保持口径一致。

---

#### A2) 选址参数化：用三元组描述“新球队落地城市”

我们用一个三元组对“扩军选址”进行参数化（实现：`project/experiments/q3_expansion_site_sensitivity.py`）：
$$
(\Delta_{market},\ \Delta_{compete},\ media\_bonus).
$$
解释：
\begin{itemize}
    \item $\Delta_{market}$：扩军对 Indiana 市场规模因子 $\mu_{\text{size}}$ 的净影响（负值代表市场稀释/观众分流；正值代表联盟关注提升带来外溢）。
    \item $\Delta_{compete}$：扩军对 Indiana 本地竞争强度 $\text{Compete}_{local}$ 的影响（越大代表附近竞品增加、注意力份额被抢占）。
    \item $media\_bonus$：扩军年媒体合作/转播关注对当期媒体收入与品牌增长的提升幅度。
\end{itemize}

我们在实验中使用 4 个候选城市情景（代码中 `SITES` 常量）：
- Toronto: $(+0.02, 0, 0.05)$  
- Denver: $(-0.02, 1, 0.02)$  
- Nashville: $(-0.03, 1, 0.01)$  
- SanDiego: $(-0.01, 0, 0.03)$  

---

#### A3) 扩军冲击如何写入环境状态（一次性跳变 + 噪声）

扩军是赛季边界发生的离散外生冲击，因此我们将其写入环境状态 $\mathcal{E}_t$ 并在进入扩军赛季的 Offseason 时触发一次（实现：`project/mdp/transitions_env.py`，调用顺序实现：`project/mdp/env.py`）。
...
$$
解释：$\mu_t$ 对应代码 `EnvState.mu_size`，进入门票收入的市场放大项；$compete_t$ 对应 `EnvState.compete_local`，进入门票需求的竞争稀释项；$\text{clip}(\cdot)$ 用于保持变量在合理范围内（防止数值爆炸）。
...
$$
解释：$N_{\text{Star,FA}}$（`EnvState.n_star_fa`）表示当期自由市场顶级球员的供给量，扩军会降低供给（默认 $\Delta_{\text{Star,FA}}=-2$）；$\text{Bidding}$（`EnvState.bidding_intensity`）表示竞价强度，扩军会提高竞价（默认 $\Delta_{\text{bidding}}=+2$）；噪声项 $\varepsilon_{FA},\varepsilon_{bid}$ 用于刻画扩军年的不确定性（供给波动、竞价波动）。

数据校准后 Indiana 的基准环境量级约为：$\mu_0\approx 0.615$，$compete_0=0$，$N_{\text{Star,FA},0}=3$，$\text{Bidding}_0=5$（来自 `calibrate_config_from_data()` 的输出）。

---

#### A4) 竞技传播链：稀缺与竞价如何改变“补强效率”

扩军不会直接更改胜率，而是通过“补强效率下降”改变竞技状态 $\mathcal{R}_t$ 的演化。阵容更新（休赛期/交易期）在代码 `project/mdp/transitions_comp.py: roster_update` 中以乘法结构落地：
$$
...
$$
解释：$\Delta_{\text{roster}}(a_{\text{roster}})$ 和 $f_{\text{salary}}(a_{\text{salary}})$ 分别表示“操作方向/力度”与“薪资投入强度”对补强幅度的影响；扩军通过 $f_{\text{FA}}$（稀缺度）与 $f_{\text{bid}}$（溢价）压缩补强效率。
...
$$
解释：若扩军导致 $N_{\text{Star,FA}}$ 下降，则 $f_{\text{FA}}$ 变小；若扩军导致竞价强度 $B$ 上升，则 $f_{\text{bid}}$ 变小。以“从 $N=3,B=5$ 变为 $N=1,B=7$（忽略噪声）”为例，补强效率的相对比例约为 $0.96/1.05\approx 0.914$，即同样的补强动作在扩军年约减少 8.6\% 的边际效果。

该 $\mathbf{Q}_t$ 进一步进入赛季内胜率更新（`project/mdp/transitions_comp.py: game_sim_update`）：
$$
P(\text{win})=\sigma\big(\eta_0+\eta_1\Delta ELO-\eta_{SOS}(SOS-1)+\eta_2 Syn+\eta_3\tanh(\overline{Q})\big).
$$
解释：$\sigma(\cdot)$ 为 Sigmoid；$\overline{Q}$ 为 $\mathbf{Q}$ 的均值（球队综合能力 proxy）；扩军导致的“补强效率下降”会通过 $\overline{Q}$ 项降低赢球概率与 ELO 演化速度。

---

#### A5) 财务传播链：市场/竞争/竞价/媒体红利如何进入 CF 与 FV

财务转移在 `project/mdp/transitions_fin.py` 中实现。门票收入（gate revenue）包含市场与竞争的入口项：
$$
Rev_{\text{gate}}=\text{BaseGate}\cdot f_{\text{perf}}(Win\%)\cdot f_{\text{star}}(\overline{Q})
\cdot f_{\text{macro}}(\text{Macro})\cdot \mu_{\text{size}}\cdot f_{\text{compete}}(compete_{local})
\cdot f_{\text{ticket}}(a_{\text{ticket}})\cdot f_{\text{mkt}}(a_{\text{marketing}}).
$$
解释：$Win\%$ 与 $\overline{Q}$ 分别捕捉竞技成绩与球星效应对收入的提升；$\mu_{\text{size}}$ 放大市场规模；$f_{\text{compete}}$ 捕捉本地竞争稀释；票价与营销分别通过弹性与上座率进入。

本地竞争因子在代码中定义为：
$$
f_{\text{compete}}=\text{clip}\big(1-\beta_{\text{compete}}\cdot compete_{local}\big),\qquad \beta_{\text{compete}}=0.04.
$$
解释：当 $compete_{local}$ 上升时，门票需求按比例下降；$\beta_{\text{compete}}$ 为“竞争稀释”强度（参数在 `project/mdp/config.py` 中定义）。

竞价强度抬升薪资成本（使扩军年“高薪抢星”更贵）：
$$
...
$$
解释：当竞价强度 $\text{Bidding}$ 超过基准 $B_0=5$ 时，Payroll 出现额外溢价项；$\beta_{\text{bid}}=0.03$（参数在 `project/mdp/config.py` 中定义）。例如 $\text{Bidding}:5\to 7$ 时，Payroll 约增加 $1+0.03\cdot 2=1.06$（+6\%），从而压缩现金流。

媒体 bonus 在扩军年通过“媒体收入 + 估值增长”双通道注入：
$$
Rev_{\text{media}}=\text{BaseMedia}\cdot f_{\text{macro}}\cdot(1+media\_bonus)\quad \text{if } I_{\text{Expansion}}=1,
$$
解释：扩军年 $I_{\text{Expansion}}=1$ 时，媒体收入按 $(1+media\_bonus)$ 放大。

同时估值增长率在扩军年增加保守加成：
$$
V_t\leftarrow V_t+0.5\cdot media\_bonus\quad \text{if } I_{\text{Expansion}}=1.
$$
解释：扩军通常提升联盟关注度与品牌叙事，因此我们给予估值增长一个较小加成（幅度低于“完整媒体周期 spike”，避免过度乐观）。

---

#### A6) 联盟层“受益/受损”判定：可解释代理评分

题目要求指出“扩军下哪些球队所有者更不利/更有利”。由于缺乏所有球队的公开财务报表，我们使用可解释的联盟层代理评分（实现：`project/experiments/q3_expansion_site_sensitivity.py:_league_impact`）。

市场规模指数（attendance proxy）：
$$
\mu_i = \frac{AvgAttendance_i}{LeagueAvg}.
$$
解释：$\mu_i>1$ 表示该队市场更大、上座更强，因此媒体红利与市场变化的放大效应更显著。

强度指数（player-based strength proxy）：
$$
s_i = \frac{ELO_i - 1500}{100}.
$$
解释：$s_i$ 越大代表球队整体强度更高；在自由市场稀缺冲击下，弱队更难通过签约补强，因此需要用 $s_i$ 区分“稀缺对不同球队的伤害程度”。
...
- 0.02\,Scarcity(1-s_i).
$$
解释：前两项表示媒体红利与市场变化对大市场队的放大效应；第三项表示竞争加剧更伤害小市场队；$LocalPenalty$ 表示扩军城市附近球队的市场重叠惩罚；$Scarcity=|\Delta_{\text{Star,FA}}|$ 表示自由市场稀缺强度；最后一项表示稀缺对弱队更不利。

---

### B. 算法实现方法（PPO 学习 + 情景对比 + 证据输出）

#### B1) 代码实现位置（从公式到代码的映射）

\begin{itemize}
    \item 扩军冲击：`project/mdp/transitions_env.py`（一次性写入 $\mu,compete,N_{\text{Star,FA}},Bidding$）。
    \item 竞技传播链：`project/mdp/transitions_comp.py`（$f_{\text{FA}},f_{\text{bid}}$ 影响 roster\_update；$\overline{Q}$ 影响胜率）。
    \item 财务传播链：`project/mdp/transitions_fin.py`（$\mu$、$compete$ 进入门票收入；$Bidding$ 进入 payroll；$media\_bonus$ 进入媒体收入与估值增长）。
    \item 扩军 Q3 实验：
    \begin{itemize}
        \item 选址敏感性：`project/experiments/q3_expansion_site_sensitivity.py`
        \item 策略路径对比（Baseline / Aggressive / Defensive / PPO）：`project/experiments/q3_expansion_policy_comparison.py`
    \end{itemize}
    \item 输出文件（数值证据）：
    \begin{itemize}
        \item `project/experiments/output/q3_expansion_sensitivity.csv`
        \item `project/experiments/output/q3_policy_comparison_summary.csv`
        \item `project/experiments/output/q3_policy_comparison_detail.csv`
    \end{itemize}
\end{itemize}

#### B2) 实验设定（统一约束与可复现性）

为聚焦“扩军冲击对杠杆/薪资/补强决策的影响”，Q3 的两组实验均采用：
\begin{itemize}
    \item 扩军年份固定为 2026（只触发一次扩军），以避免多次扩军叠加导致解释困难；
    \item 关闭股权稀释（`max_equity_action=0`），使结果主要由“债务杠杆 + 薪资/阵容”决定；
    \item 全程遵守 Phase-aware Action Mask（Offseason 可重构，赛季内受冻结约束），保证操作窗口合法；
    \item PPO 的动作集合使用 allowed mask（与环境约束一致），保证训练策略始终可执行。
\end{itemize}

两组实验的 PPO 训练/评估参数（与代码一致）：
\begin{itemize}
    \item \textbf{选址敏感性（`q3_expansion_site_sensitivity.py`）}：steps\_per\_update=128，epochs=2；训练 episode=8；评估 episode=6；max\_steps=20（对应 5 个赛季，每赛季 4 个 phase step）。
    \item \textbf{策略路径对比（`q3_expansion_policy_comparison.py`）}：steps\_per\_update=256，epochs=2；训练 episode=8；评估 seeds=6；评估赛季数=3（共 12 个 phase step），并记录扩军年 Offseason 的动作分布用于证据化分析。
\end{itemize}

---

### C. 结果分析（用数据检验：选址影响 + 策略调整 + 受益/受损）

#### C1) 选址敏感性（Indiana：Baseline vs PPO）

我们先回答“新球队落地城市如何影响 Indiana 的价值/利润”。实验（`q3_expansion_site_sensitivity.py`）对每个选址训练 PPO，并与不调整的基线策略对比，输出 Indiana 所有者的平均现金流与终值（见 `q3_expansion_sensitivity.csv`；对应图：`newfigures/q3_expansion_bar.png`）。

表：不同选址下 Indiana 的终值与现金流（PPO 相对 Baseline）。

| Site | Baseline Terminal | PPO Terminal | ΔTerminal | Baseline CF | PPO CF | ΔCF |
|---|---:|---:|---:|---:|---:|---:|
| Nashville | 493.685 | 629.897 | +136.212 | 16.544 | 18.378 | +1.834 |
| Toronto | 537.895 | 653.108 | +115.213 | 17.780 | 18.140 | +0.360 |
| Denver | 504.503 | 618.403 | +113.900 | 16.720 | 18.356 | +1.636 |
| SanDiego | 515.476 | 596.649 | +81.172 | 17.264 | 17.519 | +0.255 |

解释：表中的 CF 为“每个决策步（phase step）的平均经营现金流”，Terminal 为所有者终值；$\Delta$ 表示 PPO 在扩军冲击下对基线策略的改进幅度。

结论：
\begin{itemize}
    \item \textbf{绝对水平}：在 PPO 策略下，Toronto 的终值最高（653.108），SanDiego 最低（596.649）；说明“市场外溢 + 媒体红利”的选址更利于 Indiana 的长期价值。
    \item \textbf{调整空间}：Nashville 的基线终值最低（493.685），但在 PPO 自适应调整后提升最大（+136.212），说明不利选址并非“无解”，关键在于策略必须随扩军冲击调整（见下一节）。
\end{itemize}

---

#### C2) 扩军赛季 Indiana 应如何调整策略（Aggressive vs Defensive vs PPO 的定量对比）

仅给出“应当去杠杆/稳阵容”的定性结论是不够的，因此我们进一步做策略路径对比实验（`q3_expansion_policy_comparison.py`），比较四种策略在扩军冲击下 3 个赛季后的关键指标（见 `q3_policy_comparison_summary.csv`；对应图：`newfigures/q3_policy_comparison_deltas.png`）。

四类策略定义：
\begin{itemize}
    \item Baseline：默认固定策略（mask-aware + nearest-valid）。
    \item Aggressive：高薪抢星 + 激进补强 + 债务加杠杆（动作目标 $(6,5,6,3,4,0)$，若触发风险约束则投影为最近可行动作）。
    \item Defensive：阵容稳定 + 适度去杠杆（动作目标 $(3,1,2,3,1,0)$）。
    \item PPO：在相同约束下训练得到的自适应策略。
\end{itemize}

表：第 3 赛季末（Season 3）关键指标（均值 ± 标准差，6 个随机种子）。

| Site | Policy | Win% | CumCF | Terminal | Debt | Leverage |
|---|---|---:|---:|---:|---:|---:|
| Denver | Baseline | 0.200±0.017 | 193.450±8.218 | 431.447±20.968 | 67.000±0.000 | 0.135±0.006 |
| Denver | PPO | 0.185±0.026 | 221.448±6.168 | 531.573±17.970 | 0.000±0.000 | 0.000±0.000 |
| Denver | Defensive | 0.190±0.017 | 220.414±6.691 | 533.741±22.316 | 0.000±0.000 | 0.000±0.000 |
| Denver | Aggressive | 0.102±0.012 | 83.860±10.832 | 295.710±18.318 | 179.477±0.817 | 0.378±0.015 |
| Nashville | Baseline | 0.200±0.017 | 191.539±8.170 | 421.966±20.587 | 67.000±0.000 | 0.137±0.006 |
| Nashville | PPO | 0.161±0.018 | 211.630±5.687 | 495.971±17.539 | 0.000±0.000 | 0.000±0.000 |
| Nashville | Defensive | 0.190±0.017 | 218.587±6.649 | 523.644±21.913 | 0.000±0.000 | 0.000±0.000 |
| Nashville | Aggressive | 0.102±0.012 | 81.769±10.795 | 286.620±17.978 | 179.477±0.817 | 0.386±0.015 |
| SanDiego | Baseline | 0.200±0.017 | 198.993±8.353 | 441.066±21.354 | 67.000±0.000 | 0.132±0.006 |
| SanDiego | PPO | 0.191±0.032 | 226.825±5.706 | 541.371±16.675 | 0.000±0.000 | 0.000±0.000 |
| SanDiego | Defensive | 0.190±0.017 | 225.749±6.809 | 543.983±22.724 | 0.000±0.000 | 0.000±0.000 |
| SanDiego | Aggressive | 0.102±0.012 | 89.278±10.950 | 304.932±18.663 | 179.477±0.817 | 0.371±0.014 |
| Toronto | Baseline | 0.200±0.017 | 204.437±8.492 | 460.718±22.143 | 67.000±0.000 | 0.127±0.006 |
| Toronto | PPO | 0.195±0.038 | 230.802±6.932 | 562.504±25.618 | 0.000±0.000 | 0.000±0.000 |
| Toronto | Defensive | 0.190±0.017 | 230.969±6.932 | 564.909±23.559 | 0.000±0.000 | 0.000±0.000 |
| Toronto | Aggressive | 0.102±0.012 | 94.913±11.065 | 323.778±19.366 | 179.477±0.817 | 0.357±0.014 |

解释：Win% 为模型内部的胜率 proxy（由 ELO/能力/协同推导）；CumCF 为 3 个赛季累计现金流；Debt/Leverage 为终点的债务与杠杆水平。

核心结论（回答“扩军年如何调整”）：
\begin{itemize}
    \item \textbf{扩军年不应走“高杠杆 + 高薪抢星”路径}：Aggressive 在所有选址下都显著降低终值与现金流（例如 Nashville：Terminal 286.6 显著低于 Baseline 422.0 与 Defensive 523.6），其机制来自 A4–A5：$N_{\text{Star,FA}}\downarrow$、$\text{Bidding}\uparrow$ 使补强效率下降且 payroll 溢价上升，叠加高债务利息与风险惩罚，导致“花更多钱却买不到同比例的胜率提升”。
    \item \textbf{扩军年应当以现金流稳定 + 去杠杆 + 稳阵容为优先}：PPO 与 Defensive 在所有选址下都把 Debt 压到 0，并显著抬升 Terminal 与 CumCF（如 Toronto：Terminal 从 460.7 提升到 562.5–564.9，CumCF 从 204.4 提升到 230.8–231.0）。
    \item \textbf{竞技表现与财务健康存在权衡，但可控}：在本组权重下 PPO/Defensive 的 Win% 与 Baseline 相近或略低（例如 Denver：0.185–0.190 vs 0.200），说明“追求所有者终值最大化”会倾向保守财务，但并未出现不可接受的竞技崩溃；若题目强调“更高胜率”，可在 Q1 的 reward 权重中提高 $w_{win\_pct}$ 以重新平衡（本节不展开）。
\end{itemize}

扩军年 Offseason 的策略变化（证据：`q3_policy_comparison_detail.csv`，对应图：`newfigures/q3_offseason_action_debt.png`、`newfigures/q3_offseason_action_salary.png`）：
\begin{itemize}
    \item Baseline 在 2026 Offseason 基本维持 $(a_{\text{debt}},a_{\text{salary}},a_{\text{roster}})=(2,2,3)$；
    \item PPO 在 2026 Offseason 的平均动作更偏向去杠杆与中等薪资投入，例如 Nashville：$a_{\text{debt}}=1.17\pm 0.41$，$a_{\text{salary}}=3.00\pm 1.26$，$a_{\text{roster}}=4.00\pm 0.00$；
    \item 该行为层证据与 A4–A5 的机制层解释一致：扩军年“高薪补强的边际效率下降 + payroll 溢价上升”，因此最优策略会减少杠杆冲动并控制薪资结构风险。
\end{itemize}

---

#### C3) 哪些球队所有者/选址更不利或更有利（联盟层）

联盟层代理指标 $Impact_i$（见 A6）可输出“各选址下 Top3/Bottom3 受益/受损球队”（可视化：`newfigures/q3_chord_expansion.png`）。在本数据与设定下：
\begin{itemize}
    \item Toronto 情景：受益前三为 PHO(+0.0407), LAS(+0.0296), SEA(+0.0278)；受损前三为 CON(-0.0442), WAS(-0.0188), DAL(-0.0156)。
    \item Nashville 情景：由于本地重叠惩罚（LocalPenalty）与竞争稀释，更不利的队包含 WAS(-0.1250), ATL(-0.1130), IND(-0.0987)。
\end{itemize}
解释：大市场球队在媒体红利项 $0.6\,media\_bonus\,\mu_i$ 与市场变化项 $\Delta_{market}\mu_i$ 上更占优；而本地竞争加剧与自由市场稀缺度上升（Scarcity）对小市场/弱队更不利，因此扩军选址会显著改变联盟内部“受益/受损”的结构。
