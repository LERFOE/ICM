...
我们以 Indiana Fever 2024 赛季阵容为基准，设定总股权上限 $\Omega_{cap}=4\%$ （后面会解释4%是怎么来的）进行求解。模型给出了明确的“给谁”与“给多少”建议（共 4 名球员获得股权）：
...
- 曲线呈现 **单调递增但斜率递减** 的特征。
- 在 4% 出现明显的“膝点（Knee Point）”。这意味着 4% 是一个性价比极高的预算上限——既能覆盖最核心的激励需求，又不会造成过度的所有权外流。这验证了我们设定 $\Omega_{cap}=4\%$ 的合理性。
